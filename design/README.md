...
| `MOBILE_REFERENCE.md` | § 6 — **the tap-target floor is 44px, not 40.** A `@media (max-width:767px)` block in `globals.css` floors **form controls only** (`input`/`select`/`textarea`); **`button` is deliberately excluded** so § 0.8's owner-granted 36px carve-out on the carousel arrows survives, and buttons carry `min-h-[44px] md:min-h-0` at the call site. | 2026-08-24 |
| `MOBILE_REFERENCE.md` | § 1 / § 3 / § 4 / § 5 — **the named machinery does not exist here**: no `useIsMobile()`, `mobile-cards.tsx`, `.ap-chip-strip`. Patterns kept, claims corrected. | 2026-08-24, owner |
| `MOBILE_REFERENCE.md` | § 8 / § 9 — **`lint:mobile` now EXISTS** (built 2026-08-24 on the owner's instruction) but with **five** rules, not six, and `tap-target` / `fixed-width` differ from upstream's; `modal-width` is deliberately unbuilt until a modal exists to exercise it. | 2026-08-24, owner |
