...
function buildSchedule() {
  ...
  rows.push({ month: 0, investment: -PRINCIPAL, dealFee: 0, interest: 0, totalReturn: 0, prinReturned: 0, globalReturn: -PRINCIPAL });
  ...
  for (let m = 1; m <= MONTHS; m++) {
    ...
    balance -= principal;
    rows.push({ month: m, investment: 0, dealFee, interest, totalReturn, prinReturned: principal, globalReturn });
  }
  ...
  return (
    <div style={{ marginTop: 12 }}>
      <button onClick={() => setOpen(!open)} style={{
        ...
        padding: "8px 14px", background: "rgba(251,191,36,0.05)",
        border: "1px solid rgba(251,191,36,0.15)", borderRadius: open ? "8px 8px 0 0" : 8,
      ...
      {open && (
        <div style={{ border: "1px solid rgba(0,0,0,0.07)", borderTop: "none", borderRadius: "0 0 8px 8px", overflow: "hidden" }}>
          <div style={{ overflowX: "auto" }}>
            <div style={{ display: "grid", gridTemplateColumns: colWidths, padding: "6px 14px", background: "rgba(56,189,248,0.05)", borderBottom: "1px solid rgba(0,0,0,0.06)", minWidth: "fit-content" }}>
              {cols.map(c => <span key={c} style={{ fontSize: 8, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".08em", color: "#94a3b8", whiteSpace: "nowrap" }}>{c}</span>)}
            </div>
            {schedule.map((r, idx) => (
              <div key={r.month} style={{ display: "grid", gridTemplateColumns: colWidths, padding: "8px 14px", alignItems: "center", borderBottom: "1px solid rgba(0,0,0,0.04)", background: idx % 2 === 0 ? "transparent" : "rgba(0,0,0,0.015)", minWidth: "fit-content" }}>
                <span style={cellStyle}>{r.month === 0 ? "0" : r.month}</span>
                <span style={{ ...cellStyle, color: r.investment < 0 ? "#f87171" : "#0f172a" }}>{r.investment !== 0 ? fmt(r.investment) : "—"}</span>
                <span style={cellStyle}>{r.dealFee > 0 ? fmt(r.dealFee) : "—"}</span>
                <span style={cellStyle}>{r.interest > 0 ? fmt(r.interest) : "—"}</span>
                <span style={{ ...cellStyle, color: r.totalReturn > 0 ? "#16a34a" : "#0f172a" }}>{r.totalReturn > 0 ? fmt(r.totalReturn) : "—"}</span>
                <span style={cellStyle}>{r.prinReturned > 0 ? fmt(r.prinReturned) : "—"}</span>
                <span style={{ ...cellStyle, fontWeight: 700, color: r.globalReturn < 0 ? "#f87171" : r.globalReturn > 0 ? "#16a34a" : "#0f172a" }}>{fmt(r.globalReturn)}</span>
              </div>
            ))}
            <div style={{ display: "grid", gridTemplateColumns: colWidths, padding: "8px 14px", background: "rgba(56,189,248,0.04)", borderTop: "2px solid rgba(56,189,248,0.2)", minWidth: "fit-content" }}>
          ...
          <div style={{ padding: "10px 14px", borderTop: "1px solid rgba(0,0,0,0.06)", background: "rgba(74,222,128,0.03)", display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".1em", color: "#64748b" }}>Annual IRR</span>
            <span style={{ fontSize: 15, fontWeight: 800, color: "#16a34a", fontVariantNumeric: "tabular-nums" }}>{(annualIRR * 100).toFixed(2)}%</span>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div style={{ background: "#f8fafc", minHeight: "100vh" }}>
      <header style={{
        ...
        <span style={{ fontSize: 12, fontWeight: 800, letterSpacing: ".06em", textTransform: "uppercase", color: "#0f172a" }}>Savoy Capital Fund</span>
        <butto
